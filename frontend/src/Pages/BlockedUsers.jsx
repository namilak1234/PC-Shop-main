...
// BlockedUsers component definition
const BlockedUsers = () => {
  // State to manage the list of blocked users
  ...
  // Fetch blocked users from the API when the component mounts
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setLoading(true);
        const response = await axios.get('/api/auth/users/blocked');
  ...
  const handleUnblockUser = async (userId) => {
    if (window.confirm("Are you sure you want to unblock this user?")) {
      try {
        await axios.put(`/api/auth/users/unblock/${userId}`);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="relative">
          <input
            type="text"
            placeholder="Enter email or status"
